...
파일 및 데이터가 실수로 다른 파트너가 소유한 대상 데이터 소스에 온보딩되는 것을 방지하기 위해 Audience Manager은 파트너 ID(PID)와 다른 파트너가 소유한 데이터 소스(DPID) 간의 매핑 요구 사항을 추가했습니다. [Audience Manager ID 인덱스](https://experienceleague.adobe.com/docs/audience-manager/user-guide/reference/ids-in-aam.html?lang=ko)에서 PID 및 DPID에 대해 자세히 알아보십시오.
...
이러한 매핑을 생성하려면 Jira 티켓을 Audience Manager 엔지니어링 팀에 제출하십시오. 샘플 Jira 티켓 [여기](https://jira.corp.adobe.com/browse/AAM-60353)를 확인하십시오. 기존 데이터 공유 관계에 대해 매핑을 생성하도록 요청할 필요가 없습니다.
