...
* [!DNL Audience Manager]이(가) [!UICONTROL Data Source]파트너 이름`*`이라는 `*-Company Device Graph-Person`을(를) 자동으로 만듭니다. [!UICONTROL Data Source] 세부 정보 페이지에서 [!DNL Audience Manager] 고객은 파트너 이름, 설명을 변경하고 이 데이터 원본에 [데이터 내보내기 제어](https://experienceleague.adobe.com/docs/device-co-op/using/device-graph/links.html?lang=en)를 적용할 수 있습니다.
